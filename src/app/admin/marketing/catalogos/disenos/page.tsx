...
export default async function DisenosPage() {
  ...
  const [plantillas, fijas, enlaces, marcas] = await Promise.all([
    plantillasGestion(),
    fijasGestion(),
    enlacesDeContacto(),
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <BibliotecaDisenos base={IMAGENES_BASE} plantillas={plantillas} fijas={fijas} marcas={marcas.rows.map((r) => r.marca as string)} enlaces={enlaces} />
    </div>
  );
}
