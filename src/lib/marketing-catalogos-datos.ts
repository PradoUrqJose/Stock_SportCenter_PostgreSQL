...
export async function fijasDeBiblioteca(): Promise<FijaBiblioteca[]> {
  const r = await db.execute(
    "SELECT id, nombre, tipo, imagen, ancho, alto, auto_tipo, auto_posicion, marca FROM mk_paginas_fijas WHERE activa = 1 ORDER BY orden, id"
  );
...
export async function fijasGestion(): Promise<(FijaBiblioteca & { activa: boolean; zonas: ZonaEnlace[] })[]> {
  const r = await db.execute(
    "SELECT id, nombre, tipo, imagen, ancho, alto, auto_tipo, auto_posicion, marca, activa, zonas FROM mk_paginas_fijas ORDER BY orden, id"
  );
  return r.rows.map((f) => ({
